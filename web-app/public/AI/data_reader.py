...
def get_train_data():
    files=glob.glob(os.path.join(data_dir,'D:\\imagedata\\train\\**\\**.jpg'))
    data = []
    mel = []
    nv = []
    bcc = []
...
def get_validation_data():
    files=glob.glob(os.path.join(data_dir, 'D:\\imagedata\\validation\\**\\**.jpg'))
...
def get_validation_loader_for_upload(batch_size):
    test_files=glob.glob("D:\\imagedata\\validation\\**\\**.jpg")
...
def get_test_loader_for_upload(batch_size):
    test_files=glob.glob("D:\\imagedata\\test\\**\\**.jpg")
    test_data_set = DatasetReader(test_files,'test')
    testloader = torch.utils.data.DataLoader(test_data_set, batch_size=batch_size, shuffle=False,
                                              num_workers=2)
    return testloader